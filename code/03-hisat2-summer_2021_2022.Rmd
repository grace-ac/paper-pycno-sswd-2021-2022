...
read in csv and subset the libraries from experiment B for gene counts
```{r}
eBtranscript8v8Counts <- read.csv("../data/transcript_count_matrix.csv", row.names = "transcript_id")
...
Write out differentially expressed gene list: 
```{r}
#write.table(exBt.sig, "../data/expB_DEG_DET_lists/experimentB_8Ev8C_DETlist.tab", sep = '\t', row.names = T)
```
...
read in csv and subset the libraries from experiment B for gene counts
```{r}
eBgene8v8Counts <- read.csv("../data/gene_count_matrix.csv", row.names = "gene_id")
