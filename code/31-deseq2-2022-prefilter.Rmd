---
title: "31-deseq2-2022-prefilter"
...
filter out genes with counts <10 for 3 samples: 
```{r}
smallestGroupSize <- 3
...
ddspf <- DESeqDataSetFromMatrix(countData = counts22sub,
                              colData = colData, design = ~ condition)


# Run the default analysis for DESeq2 and generate results table
ddspf <- DESeq(ddspf)
deseq2.res <- results(ddspf)

# Sort by adjusted p-value and display
resOrdered <- deseq2.res[order(deseq2.res$padj), ]
vsd <- vst(ddspf, blind = FALSE)
...
counts <- counts(ddspf, normalized = TRUE)
...
```{r}
smallestGroupSize <- 3
keep <- rowSums(counts(ddsCvEwA) >= 10) >= smallestGroupSize
ddsCvEwApf <- ddsCvEwA[keep,]
```
...
```{r}
design(ddsCvEwApf) <- formula(~ age + condition)
ddsCvEwApf <- DESeq(ddsCvEwApf)
```

Access results:
```{r}
ddsCvEwA.res <- results(ddsCvEwApf)
head(ddsCvEwA.res)
```
...
```{r}
resultsNames(ddsCvEwApf)
```

```{r}
deseq2.resage <- results(ddsCvEwApf,
...
smallestGroupSize <- 3
keep <- rowSums(counts(ddsCvEwA) >= 10) >= smallestGroupSize
ddsCvEwAfil <- ddsCvEwA[keep,]
```


```{r}
ddsCvEwAfil <- DESeq(ddsCvEwAfil)
resultsNames(ddsCvEwAfil)
```


Check levels of age and condition 
```{r}
levels(ddsCvEwAfil$age)
```


```{r}
levels(ddsCvEwAfil$condition)
```
...
res1 <- results(ddsCvEwAfil, 
                contrast = contraster(ddsCvEwAfil,
...
```{r}
# adult vs juvenile in exposed group 
diff1 <- contraster(ddsCvEwAfil,
                    group1 = list(c("age", "adult"), c("condition", "exposed")),
                    group2 = list(c("age", "juvenile"), c("condition", "exposed")))

# adult vs juvenile in control group
diff2 <- contraster(ddsCvEwAfil,
                    group1 = list(c("age", "adult"), c("condition", "control")),
                    group2 = list(c("age", "juvenile"), c("condition", "control")))

res5 <- results(ddsCvEwAfil,
