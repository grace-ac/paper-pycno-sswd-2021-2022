...
# Junuary 2026 -- pre-filtering before running deseq
Read in gene count matrix:
```{r}
counts21 <- read.csv("../data/gene_count_matrix_2021.csv")
head(counts21)
...
remove any rows where there are counts <10 for more than 8 samples: 
```{r}
smallestGroupSize <- 8
...
```{r}
# Create a DESeqDataSet from count matrix and labels
ddspf <- DESeqDataSetFromMatrix(countData = exp,
                              colData = colData, design = ~ condition)
...
abline(h=c(-1,1), col="blue")
```
Pre-filtering for this doesn't change anything -- so it didn't actually end up needing to filter anything out. 
