...
```{r, engine='bash', eval=TRUE}
head ../analyses/13-hisat-deseq2/hisat.out
```

```{r, engine='bash', eval=TRUE}
cat ../analyses/13-hisat-deseq2/hisat.out \
...
-T \
-o ../analyses/13-hisat-deseq2/augustus.hint.2.gff
...
-p 36 \
-eB \
-G ../analyses/13-hisat-deseq2/augustus.hint.2.gff \
-o ../analyses/13-hisat-deseq2/{}.gtf \
../analyses/13-hisat-deseq2/{}.sorted.bam'
```

```{bash}
head ../analyses/13-hisat-deseq2/*gtf
...
```{r, engine='bash', eval=TRUE}
ls ../analyses/13-hisat-deseq2/*gtf
```
...
-i ../data/list01.txt \
-g ../analyses/13-hisat-deseq2/gene_count_matrix.csv \
-t ../analyses/13-hisat-deseq2/transcript_count_matrix.csv
```

```{r, engine='bash', eval=TRUE}
head ../analyses/13-hisat-deseq2/*matrix.csv
```
...
countData <- as.matrix(read.csv("../analyses/13-hisat-deseq2/gene_count_matrix.csv", row.names="gene_id"))
...
```{r}
write.table(tmp.sig, "../analyses/13-hisat-deseq2/DEGlist.tab", sep = '\t', row.names = T)
```

```{r, eval=TRUE}
deglist <- read.csv("../analyses/13-hisat-deseq2/DEGlist.tab", sep = '\t', header = TRUE)
